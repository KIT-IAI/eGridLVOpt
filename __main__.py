...
class HumanPreference:
    """
    Stores user-defined preferences for simulation and optimization.
    Parameters:
    ----------
    horizon_length : int
        Prediction horizon (timesteps).
    omega_1 : float
        Weight for first objective.
    omega_2 : float
        Weight for second objective.
    pywraplp_solver : str
        Solver name for OR-Tools.
    initial_soc_solver : float
        Initial SoC for optimization (%).
    target_soc_solver : float
        Target SoC at horizon end (%).
    initial_soc_sim : float
        Initial SoC for simulation (%).
    noise_percent : float
        Input noise level (%).
    solving_method : str
...
class manager_solver:
    ...
    def __init__(self,
        ...
        for step_temp in range(self.simulation_steps):
            ...
            # Instead of summing sgens and loads manually, you can use
            # self.net.res_bus["p_mw"].iloc[0] as total power at bus 0
        ...
        self.prediction_profiles_load_q_noisy = (
            self.profiles_load_q.copy().to_numpy().transpose()
        )
        self.prediction_profiles_sgen_p_noisy = (
            self.profiles_sgen_p.copy().to_numpy().transpose()
        )
        ...
        self.omega_1 = human_preference.omega_1
        # Weight 2 for selecting objective function
        self.omega_2 = human_preference.omega_2
        ...
        if self.solver_library == solver_library.pyomo:
            ...
            # Dimensioning of battery storage systems
    ...
    def simulate_step(self):
        """
        Performs a single timestep in the simulation loop, including
        optimization via OR-Tools (pywraplp) and power flow analysis
        using pandapower.
        """
        self.__update_environment()
        # Run power flow simulation after control solution for storage and heatpump
        pp.runpp(self.net)
    ...
    def __update_environment(self):
        """Update all grid state components before solving"""
        ...
        if self.solver_library == solver_library.pyomo:
            ...
            try:
                ...
                _ = result.solver.termination_condition.value  # solver_condition
    ...
    def min_p_mw_for_0_soc(self, soc, max_e_mwh):
        """Max discharge power to stay above 0% SoC."""
        return (-soc) * max_e_mwh * (60 / 15) / 100

    def solve(self):
        """Solve grid using the selected method"""
    ...
    def simulate(self):
        ...
        for i in self.net.storage.index:
            # SOCs are initialized at weird values otherwise
            self.net.storage.soc_percent[i] = self.initial_soc_sim
...
def get_energy_prices(year):
    """
    Loads, processes, and resamples spot market energy prices for the given year.
    Parameters:
    year : int or str
        Year of the energy price data to load.
    Returns:
    np.ndarray
...
def main():
    """Main function to run the solver manager. handles debug and CLI mode"""
    debug = False
    if debug:
