...
a non-linear AC optimal power flow (OPF) problem with optional
...
class NonLinearModel:
    def __init__(
        ...
        assert (
            objective in allowed_objectives
        ...
        if objective == "min_cost" and self.Enet.poly_cost.empty:
            print(
                'No cost data in Enet.poly_cost. Objective set to "quadratic_exchange".'
            )
        ...
        assert (
            flow_constraint in allowed_flowconstr
        ...
        if self.withTimeseries:
            ...
            if set(necessary_time_series_profiles).issubset(self.Enet.keys()):
                ...
                assert (
                    self.Enet["load"].shape[0] == self.Enet["profiles_load_p"].shape[0]
                ), "Number of loads does not match the number of load profile rows."

                self.t_all = self.Enet["profiles_load_p"].shape[1]

                if not self.Enet.sgen.empty and set(
                    necessary_time_series_sgen_profiles
                ).issubset(self.Enet.keys()):
                    assert (
                        self.Enet["profiles_load_p"].shape[1]
                ...
                if not self.Enet.gen.empty and set(
                    necessary_time_series_gen_profiles
                ).issubset(self.Enet.keys()):
                    assert (
                        self.Enet["profiles_load_p"].shape[1]
                ...
                if not self.Enet.storage.empty and set(
                    necessary_time_series_sto_profiles
                ).issubset(self.Enet.keys()):
                    assert (
                        self.Enet["profiles_load_p"].shape[1]
        ...
        self.index_hp = np.where(
            self.Enet.load.profile.str.startswith(("Soil", "Air"))
        )[0]
        self.max_p_mw_hp = {}
    ...
    def transform_pandapower_grid_to_model(self):
        ...
        model.nodes_no_hp = pe.Set(
            initialize=np.array(
                [i for i in self._get_buses() if i not in self.index_hp]
            )
        )
        ...
        model.pq_nodes_no_hp = pe.Set(
            initialize=np.array(
                [i for i in self.Enet.load.index.values if i not in self.index_hp]
            )
        )
        ...
        # Line parameters
        (g_ff, b_ff, g_ft, b_ft, g_tt, b_tt, g_tf, b_tf) = self._net_to_lineparameters()
        ...
        model.inflow_set = pe.Param(
            model.nodes, within=pe.Any, default=set(), initialize=inflow_set
        )
        model.outflow_set = pe.Param(
            model.nodes, within=pe.Any, default=set(), initialize=outflow_set
        )
        ...
        model.Storage_Pmax = pe.Param(
            model.storage_nodes, initialize=self.Enet.storage["max_p_mw"].values
        )
    ...
    def edge_flow_p_from(self, model, i, j, t):
        ...
        """
        return model.p_f[i, j, t] == model.g_ff[i, j] * model.V[i, t] ** 2 + model.V[
            i, t
        ] * model.V[j, t] * (
    ...
    def edge_flow_p_to(self, model, i, j, t):
        ...
        P_t = G_tt * V_j^2 + ...
        """
        return model.p_t[i, j, t] == model.g_tt[i, j] * model.V[j, t] ** 2 + model.V[
            i, t
        ] * model.V[j, t] * (
    ...
    def edge_flow_q_from(self, model, i, j, t):
        ...
        Q_f = -B_ff * V_i^2 + ...
        """
        return model.q_f[i, j, t] == -model.b_ff[i, j] * model.V[i, t] ** 2 + model.V[
            i, t
        ] * model.V[j, t] * (
    ...
    def edge_flow_q_to(self, model, i, j, t):
        ...
        Q_t = -B_tt * V_j^2 + ...
        """
        return model.q_t[i, j, t] == -model.b_tt[i, j] * model.V[j, t] ** 2 + model.V[
            i, t
        ] * model.V[j, t] * (
    ...
    def edge_flow_i_from_real(self, model, i, j, t):
        ...
        """
        return model.i_f_real[i, j, t] == model.g_ff[i, j] * model.V[i, t] * pe.cos(
            model.Theta[i, t]
        ) - model.b_ff[i, j] * model.V[i, t] * pe.sin(model.Theta[i, t]) + model.g_ft[
            i, j
        ] * model.V[
            j, t
        ] * pe.cos(
    ...
    def edge_flow_i_from_imag(self, model, i, j, t):
        ...
        """
        return model.i_f_imag[i, j, t] == model.b_ff[i, j] * model.V[i, t] * pe.cos(
            model.Theta[i, t]
        ) + model.g_ff[i, j] * model.V[i, t] * pe.sin(model.Theta[i, t]) + model.b_ft[
            i, j
        ] * model.V[
            j, t
        ] * pe.cos(
    ...
    def edge_flow_i_to_real(self, model, i, j, t):
        ...
        """
        return model.i_t_real[i, j, t] == model.g_tt[i, j] * model.V[j, t] * pe.cos(
            model.Theta[j, t]
        ) - model.b_tt[i, j] * model.V[j, t] * pe.sin(model.Theta[j, t]) + model.g_tf[
            i, j
        ] * model.V[
            i, t
        ] * pe.cos(
    ...
    def edge_flow_i_to_imag(self, model, i, j, t):
        ...
        """
        return model.i_t_imag[i, j, t] == model.b_tt[i, j] * model.V[j, t] * pe.cos(
            model.Theta[j, t]
        ) + model.g_tt[i, j] * model.V[j, t] * pe.sin(model.Theta[j, t]) + model.b_tf[
            i, j
        ] * model.V[
            i, t
        ] * pe.cos(
    ...
    def storage_equation(self, model, i, t):
        ...
        return (
            model.E[i, t + 1]
            - model.E[i, t]
            - model.Psto[i, t] * (self.dt_min / 60.0)
            + self.nue * model.E[i, t] * 15 / 60.0
            == 0.0
        )
    ...
    def make_constraints(self):
        ...
        self.model.nodal_power_balance_P = pe.Constraint(
            self.model.nodes, self.model.T, rule=self.nodal_power_balance_P
        )
        self.model.nodal_power_balance_Q = pe.Constraint(
            self.model.nodes, self.model.T, rule=self.nodal_power_balance_Q
        )
        self.model.nodal_power_inj_P = pe.Constraint(
            self.model.nodes, self.model.T, rule=self.nodal_power_injection_P
        )
        self.model.nodal_power_inj_Q = pe.Constraint(
            self.model.nodes, self.model.T, rule=self.nodal_power_injection_Q
        )

        # Branch flow constraints
        self.model.edge_flow_p_from = pe.Constraint(
            self.model.edges, self.model.T, rule=self.edge_flow_p_from
        )
        self.model.edge_flow_p_to = pe.Constraint(
            self.model.edges, self.model.T, rule=self.edge_flow_p_to
        )
        self.model.edge_flow_q_from = pe.Constraint(
            self.model.edges, self.model.T, rule=self.edge_flow_q_from
        )
        self.model.edge_flow_q_to = pe.Constraint(
            self.model.edges, self.model.T, rule=self.edge_flow_q_to
        )
        self.model.edge_flow_i_from_real = pe.Constraint(
            self.model.edges, self.model.T, rule=self.edge_flow_i_from_real
        )
        self.model.edge_flow_i_from_imag = pe.Constraint(
            self.model.edges, self.model.T, rule=self.edge_flow_i_from_imag
        )
        self.model.edge_flow_i_to_real = pe.Constraint(
            self.model.edges, self.model.T, rule=self.edge_flow_i_to_real
        )
        self.model.edge_flow_i_to_imag = pe.Constraint(
            self.model.edges, self.model.T, rule=self.edge_flow_i_to_imag
        )

        # Flow or current constraints
        if self.flow_constraint in ["power", "both"]:
            self.model.line_limit_s_from = pe.Constraint(
                self.model.trafo, self.model.T, rule=self.line_limit_s_from
            )
            self.model.line_limit_s_to = pe.Constraint(
                self.model.trafo, self.model.T, rule=self.line_limit_s_to
            )

        if self.flow_constraint in ["current", "both"]:
            self.model.line_limit_i_from_con = pe.Constraint(
                self.model.lines, self.model.T, rule=self.line_limit_i_from
            )
            self.model.line_limit_i_to_con = pe.Constraint(
                self.model.lines, self.model.T, rule=self.line_limit_i_to
            )

        # Storage energy balance constraints
        if not self.Enet.storage.empty:
            self.model.storage_eq = pe.Constraint(
                self.model.storage_nodes, self.model.T, rule=self.storage_equation
            )

        # Ramping constraints for storage if specified
        if self.with_storage_ramp == "True":
            self.model.storage_ramp1 = pe.Constraint(
                self.model.storage_nodes, self.model.T2minus1, rule=self.storage_ramp1
            )
            self.model.storage_ramp2 = pe.Constraint(
                self.model.storage_nodes, self.model.T2minus1, rule=self.storage_ramp2
            )

        # Additional objectives
        if (
            self.objective == "variable_cost"
            or self.objective == "pywraplp_multiobjective_power_price"
        ):
            ...
            self.model.p_grid_pos_con = pe.Constraint(
                self.model.T, rule=lambda m, t: m.p_grid_pos[t] >= 0.0
            )
            self.model.p_grid_neg_con = pe.Constraint(
                self.model.T, rule=lambda m, t: m.p_grid_neg[t] >= 0.0
            )
            self.model.p_grid_abs_con = pe.Constraint(
                self.model.T,
                rule=lambda m, t: m.p_grid_pos[t] - m.p_grid_neg[t]
                == sum(m.P[i, t] for i in self.model.slack_nodes),
            )
        ...
        self.model.P_pos_nonneg = pe.Constraint(
            self.model.nodes, self.model.T, rule=lambda m, i, t: m.P_pos[i, t] >= 0
        )
        self.model.P_neg_nonneg = pe.Constraint(
            self.model.nodes, self.model.T, rule=lambda m, i, t: m.P_neg[i, t] >= 0
        )
        ...
        for i in self.index_hp:
            ...
            for block in range(self.e_hp_blocks):
                for t in range(block * 24, (block + 1) * 24):
                    self.e_hp[i][block] += (
                        self.Enet["profiles_load_p"][i][t + self.timestep_start]
                        * 15
                        / 60
                    )
    ...
    def make_objective(self):
        ...
        if self.objective == "quadratic_exchange":
            ...
            objective_1 = sum(
                (
                    sum(
                        -self.model.P[i, t] * self.model.preis[t] * 1000 * 15 / 60
                        for i in self.model.slack_nodes
                    )
                    - self.temp_money_spend_mean
                )
            ...
            objective_2 = sum(
                (
                    sum([self.model.P[i, t] for i in self.model.slack_nodes]) ** 2
                    - self.temp_net_sum_p_mw_mean
                )
            ...
            self.model.obj_func = (
                self.omega_1 * objective_1 + self.omega_2 * objective_2
            )
    ...
    def fix_values_timestep(self, t):
        ...
        assert (
            self.t_all >= t + self.H
        ), "Requested time horizon extends beyond available time series data."
        ...
        def constraint_heatpump_lower_energy(model, i):
            """Enforce upper energy bound for heat pump i."""
            block = t_local // 24
            e_hp_history_in_block = 0
            for tt in range(block * 24, t_local):
                e_hp_history_in_block += self.p_hp_history[i][tt] * 15 / 60
            return self.e_hp[i][block] * (
                1 - self.e_hp_tolerance
            ) <= e_hp_history_in_block + sum(
                model.Pload[i, tt] * 15 / 60 for tt in range(0, (24 - (t_local % 24)))
            )

        def constraint_heatpump_upper_energy(model, i):
            """Enforce lower energy bound for heat pump i."""
            block = t_local // 24
            e_hp_history_in_block = 0
            for tt in range(block * 24, t_local):
                e_hp_history_in_block += self.p_hp_history[i][tt] * 15 / 60
            return e_hp_history_in_block + sum(
                model.Pload[i, tt] * 15 / 60 for tt in range(0, (24 - (t_local % 24)))
            ) <= self.e_hp[i][block] * (1 + self.e_hp_tolerance)

        def constraint_heatpump_zero(model, i, tt):
            """Set heat pump i power to zero at time tt."""
        ...
        self.model.constraints_heatpump_lower_power = pe.Constraint(
            self.model.heatpumps, self.model.T, rule=constraint_heatpump_lower_power
        )
        self.model.constraints_heatpump_upper_power = pe.Constraint(
            self.model.heatpumps, self.model.T, rule=constraint_heatpump_upper_power
        )
        ...
        self.model.qload_heatpump_constraint = pe.Constraint(
            self.model.heatpumps, self.model.T, rule=constraint_set_heatpump_qload
        )
        ...
        for tt in self.model.T:
            ...
            if not self.Enet.storage.empty:
                ...
                for i in self.Enet.storage.index.values:
                    self.model.Psto[i, -1].fix(
                        self.Enet.storage["p_mw"][i], skip_validation=True
                    )
                    if "min_p_mw" in self.Enet.storage.columns:
                        if not np.isnan(self.Enet.storage["min_p_mw"][i]):
                            self.model.Psto[i, :].setlb(
                                self.Enet.storage["min_p_mw"][i]
                            )
                    if "max_p_mw" in self.Enet.storage.columns:
                        if not np.isnan(self.Enet.storage["max_p_mw"][i]):
                            self.model.Psto[i, :].setub(
                                self.Enet.storage["max_p_mw"][i]
                            )
            ...
            if not self.Enet.load.empty:
                if "profiles_load_p" in self.Enet.keys():
                    for i, p in zip(
                        self.model.pq_nodes_no_hp,
                        self.Enet["profiles_load_p"][:, t + tt],
                    ):
                        self.model.Pload[i, tt].fix(p)
                if "profiles_load_q" in self.Enet.keys():
                    for i, q in zip(
                        self.model.pq_nodes_no_hp,
                        self.Enet["profiles_load_q"][:, t + tt],
                    ):
            ...
            if not self.Enet.gen.empty:
                if "profiles_gen_p" in self.Enet.keys():
                    for i, p in zip(
                        self.model.generator_nodes,
                        self.Enet["profiles_gen_p"][:, t + tt],
                    ):
            ...
            if not self.Enet.sgen.empty:
                if "profiles_pv_p" in self.Enet.keys():
                    for i, p in zip(
                        self.model.sgen_nodes, self.Enet["profiles_pv_p"][:, t + tt]
                    ):
                        self.model.Psgen[i, tt].fix(p)
                if "profiles_pv_q" in self.Enet.keys():
                    for i, q in zip(
                        self.model.sgen_nodes, self.Enet["profiles_pv_q"][:, t + tt]
                    ):
            ...
            if (
                ...
                self.model.preis[tt].fix(
                    self.Enet["prediction_energy_prices_noisy"][t + tt]
                )
    ...
    def extract_results(self):
        ...
        res_bus["E_sto"] = (
            np.array(Esto).reshape(self.H + 1, self.Enet.storage.shape[0]).T
        )
    ...
    def update_storages(self):
        ...
        This can be called after each optimization step in a rolling
        horizon approach.
        """
        # SoC is updated as E[i,1]/Emax.
        # The factor 100 is for converting to percent.
        self.Enet.storage["soc_percent"] = (
            pe.value(self.model.E[:, 1]) / self.Enet.storage["max_e_mwh"].values * 100.0
        )
        self.Enet.storage["p_mw"] = pe.value(self.model.Psto[:, 0])

    def update_TimeSeries(
        self, profiles_load_p, profiles_load_q, profiles_pv_p, profiles_pv_q
    ):
        """
    ...
    def clear_model(self):
        """
        Clears the primary model variables for a fresh start or for
        re-initializing with new data. This sets their values to None.
        """
        self.model.P[:, :] = None
        self.model.Q[:, :] = None
        self.model.V[:, :] = None
